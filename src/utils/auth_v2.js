...
import { CheckPrivateKeyValid } from './base_v2'
import { getLocalKey } from './account_v2'
import { AESKey } from './config_v2' 
...
//校验密码是否正确
export async function checkPassword(publicKey, chainName, password){
...
export async function checkAuth(permission, password){
    ...
    let chain = getCurrentChain();
    let isValid = await checkPassword(publicKey, chain.ChainName, password)
    let res = {
        isValid: isValid,
        publicKey: publicKey
    }
